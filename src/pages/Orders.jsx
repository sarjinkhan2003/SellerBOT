...
import { Eye, FileDown, Printer, Search, Trash2, X } from "lucide-react"
...
function Orders() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      query(collection(db, "users", currentUser.uid, "orders"), orderBy("createdAt", "desc")),
      (snapshot) => setOrders(snapshot.docs.map((item) => ({ id: item.id, ...item.data() }))),
      (error) => toast.error(error.message || "Could not load orders."),
    )
  ...
  const filteredOrders = useMemo(() => orders.filter((order) => {
    ...
    if (search && ![order.orderNumber, order.customerName, order.phone, order.zone].join(" ").toLowerCase().includes(search)) return false
    return true
  }), [orders, filters])
  ...
  return (
    <section className="space-y-6">
      ...
      {filteredOrders.length ? <OrdersTable orders={filteredOrders} onView={setSelectedOrder} onInvoice={setInvoiceOrder} onPaid={markAsPaid} /> : <div className="empty-state card"><p className="empty-state-title">No orders found</p><p className="empty-state-desc">Orders will appear here after saving invoices from New Order.</p></div>}

      {selectedOrder && <OrderDetail order={selectedOrder} onClose={() => setSelectedOrder(null)} onPDF={downloadPDF} onPrint={printInvoice} onPaid={markAsPaid} onDelete={deleteOrder} />}
