...
import { filterRevenueOrders, getCollectionSummary, getPaymentBreakdown, getProfitByProduct, getRevenueBreakdown, getSalesByZone, getThisMonthSales, getTodaysSales, getTopProducts, getUnpaidOrders, getYesterdaysSales, getOrderDateValue } from "../utils/analytics.js"
...
function Dashboard() {
  ...
  const revenueOrders = useMemo(() => filterRevenueOrders(orders), [orders])
  const today = useMemo(() => getTodaysSales(revenueOrders), [revenueOrders])
  const yesterday = useMemo(() => getYesterdaysSales(revenueOrders), [revenueOrders])
  const month = useMemo(() => getThisMonthSales(revenueOrders), [revenueOrders])
  const unpaid = useMemo(() => getUnpaidOrders(revenueOrders), [revenueOrders])
  const revenue = useMemo(() => getRevenueBreakdown(revenueOrders), [revenueOrders])
  const collectionSummary = useMemo(() => getCollectionSummary(revenueOrders), [revenueOrders])
  const profitProducts = useMemo(() => getProfitByProduct(revenueOrders).slice(0, 10), [revenueOrders])
  const topProducts = useMemo(() => getTopProducts(revenueOrders), [revenueOrders])
  const zones = useMemo(() => getSalesByZone(revenueOrders).slice(0, 5), [revenueOrders])
  const payments = useMemo(() => getPaymentBreakdown(revenueOrders), [revenueOrders])
  ...
  return <section className="space-y-6"><div className="card flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-4"><div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#1D9E75] text-xl font-bold text-white">{initials(shop?.shopName)}</div><div><p className="text-sm font-semibold uppercase tracking-wide text-[#1D9E75]">{new Date().toLocaleDateString("en-GB", { weekday: "long", day: "2-digit", month: "long", year: "numeric" })}</p><h2 className="text-3xl font-semibold">Welcome back, {shop?.ownerName || currentUser?.displayName || "Seller"}!</h2><p className="text-slate-600">{shop?.shopName || "SellerBot Shop"}</p></div></div></div>{unpaid.length > 0 && <Alert text={`You have ${unpaid.length} unpaid orders totaling ৳${unpaidTotal}`} action={() => navigate("/sales?status=Unpaid")} />}{pendingDeliveries.length > 0 && <Alert text={`${pendingDeliveries.length} orders are in transit - check Delivery Inventory`} action={() => navigate("/delivery-inventory")} />}{lowStock.slice(0, 3).map((p) => <div key={p.id} className="rounded-lg border border-yellow-200 bg-yellow-50 p-3 text-sm font-semibold text-yellow-900"><AlertTriangle className="mr-2 inline h-4 w-4" />{p.name} is running low ({p.stock} left)</div>)}<WorkQueue unpaid={unpaid} unpaidTotal={unpaidTotal} pendingDeliveries={pendingDeliveries} lowStock={lowStock} /><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Metric title="Today's Sales" value={`৳${today.revenue}`} detail={`${today.count} orders`} compare={`${yesterdayCompare >= 0 ? "↑" : "↓"} ${Math.abs(yesterdayCompare)}% vs yesterday`} /><Metric title="This Month" value={`৳${month.revenue}`} detail={`${month.count} orders`} /><Metric title="Total Orders" value={revenueOrders.length} detail={`৳${revenueOrders.reduce((s, o) => s + (o.grandTotal || 0), 0)} all time`} /><button className="card text-left" onClick={() => navigate("/sales?status=Unpaid")}><p className="text-sm text-slate-500">Pending Payments</p><p className="text-3xl font-bold text-orange-600">{unpaid.length}</p><p className="text-sm text-slate-600">৳{unpaidTotal} unpaid</p></button></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Metric title="Product Revenue" value={`৳${revenue.productRevenue}`} detail="From sales only" /><Metric title="Delivery Revenue" value={`৳${revenue.deliveryRevenue}`} detail="From delivery charges" /><Metric title="Total Cost" value={`৳${revenue.totalCost}`} detail="Your product costs" /><Metric title="Gross Profit" value={`৳${revenue.grossProfit}`} detail={`${revenue.productRevenue ? ((revenue.grossProfit / revenue.productRevenue) * 100).toFixed(1) : 0}% margin`} positive={revenue.grossProfit >= 0} /></div><div className="grid gap-4 md:grid-cols-2"><Metric title="Online Collected" value={`৳${collectionSummary.onlineReceived}`} detail="Received via bKash/Nagad/Rocket" /><Metric title="COD Pending" value={`৳${collectionSummary.codPending}`} detail={`${revenueOrders.filter((o) => (o.codAmount || 0) > 0).length} COD orders`} /></div><div className="grid gap-6 xl:grid-cols-2"><Panel title="Recent Orders" action={<Link className="text-sm font-semibold text-[#1D9E75]" to="/sales">View All</Link>}><RecentOrders orders={revenueOrders.slice(0, 5)} /></Panel><Panel title="Top Products"><BarList items={topProducts} labelKey="name" value={(i) => `${i.units} units · ৳${i.revenue}`} metric="units" /></Panel><Panel title="Sales by Zone"><BarList items={zones} labelKey="zone" value={(i) => `${i.count} orders · ৳${i.revenue}`} metric="revenue" /></Panel><Panel title="Payment Method Breakdown"><div className="flex flex-wrap gap-2">{payments.map((i) => <span key={i.method} className="rounded-md bg-slate-100 px-3 py-2 text-sm font-semibold">{i.method}: {i.count} orders</span>)}</div></Panel></div><Panel title="Product Profitability"><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-sm"><thead><tr className="text-left text-slate-500"><th>Product</th><th>Units Sold</th><th>Revenue</th><th>Cost</th><th>Profit</th><th>Margin%</th></tr></thead><tbody>{profitProducts.map((p) => <tr key={p.name} className="border-t"><td className="py-2 font-semibold">{p.name}</td><td>{p.unitsSold}</td><td>৳{p.revenue}</td><td>৳{p.cost}</td><td>৳{p.profit}</td><td><span className={marginClass(p.margin)}>{p.margin}%</span></td></tr>)}</tbody></table></div></Panel><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><QuickAction icon={Plus} label="New Order" to="/new-order" /><QuickAction icon={PackagePlus} label="Add Product" to="/products" /><QuickAction icon={MapPin} label="Manage Zones" to="/delivery-zones" /><QuickAction icon={Settings} label="Shop Settings" to="/shop-settings" /></div></section>
}

function WorkQueue({ unpaid, unpaidTotal, pendingDeliveries, lowStock }) { return <div className="grid gap-4 xl:grid-cols-4"><Link className="card card-hover block" to="/orders"><p className="text-sm font-semibold text-slate-500">Order Desk</p><p className="mt-2 text-2xl font-bold">Review orders</p><p className="text-sm text-slate-600">Operational order list and invoices</p></Link><Link className="card card-hover block" to="/new-order"><p className="text-sm font-semibold text-slate-500">Manual Invoice</p><p className="mt-2 text-2xl font-bold">Create quickly</p><p className="text-sm text-slate-600">Use New Order manual mode</p></Link><Link className="card card-hover block" to="/delivery-inventory"><p className="text-sm font-semibold text-slate-500">In Transit</p><p className="mt-2 text-2xl font-bold">{pendingDeliveries.length}</p><p className="text-sm text-slate-600">Delivery orders to follow up</p></Link><Link className="card card-hover block" to="/sales?status=Unpaid"><p className="text-sm font-semibold text-slate-500">Collections</p><p className="mt-2 text-2xl font-bold">৳{unpaidTotal}</p><p className="text-sm text-slate-600">{unpaid.length} unpaid orders · {lowStock.length} low stock</p></Link></div> }
